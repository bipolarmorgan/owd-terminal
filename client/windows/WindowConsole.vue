...
<script>
  // jQuery + Vue? a facc ro cazz
  window.jQuery = require('jquery');

  // jCubic Terminal integration
  ...
  export default {
    ...
    mounted() {
      const terminalElement = window.jQuery('#console');

      const terminalInstance = terminalElement.terminal(
          this.$terminal.commands,
          {
            greetings: this.$terminal.greetings,
            prompt: '> ',
            checkArity: false,
      ...
      this.$terminal.setInstance(terminalInstance);
    },
    destroyed() {
